  ...
  return (
    <div className=" bg-gradient-to-br from-gray-300 via-white to-gray-100 backdrop-blur-md border border-white/20 rounded-xl shadow-lg p-3 sm:p-4 md:p-5 lg:p-6 space-y-4 xs:space-y-5 sm:space-y-6 md:space-y-7 lg:space-y-8 my-6 sm:my-8 md:my-10 md:w-8/10 mx-auto relative">
      ...
      <div className="grid grid-cols-2 gap-1 sm:gap-1 md:gap-1.5 lg:gap-2 responsive-text-sm text-info-content">
        <p><span className="font-semibold">Client: </span> {clientResponseDTO?.firstName}</p>
        <p><span className="font-semibold">Date: </span> {new Date(orderDate)?.toLocaleString()}</p>
        ...
        <p><span className="font-semibold">Sum: </span> €{totalAmount?.toFixed(2)}</p>
      </div>
      ...
      <div>
        ...
        <div className="overflow-x-auto">
          ...
          <tr key={index} className="border-t  border-gray-400 text-info-content">
            <td className="px-1 sm:px-2 md:px-3  lg:px-4 py-2 text-center responsive-text-sm">{item?.productResponseDTO?.name ?? "N/A"}</td>
            <td className="px-1 sm:px-2 md:px-3 lg:px-4 py-2 flex justify-center items-center"><img src={item?.productResponseDTO?.imageUrl ?? "N/A"} alt="orderItemImage" className="w-30 xs:w-35 sm:w-40 md:w-45 lg:w-50 rounded-md shadow-lg" /></td>
            <td className="px-1 sm:px-2 md:px-3 lg:px-4 py-2 text-center responsive-text-sm">{item?.quantity}</td>
            <td className="px-1 sm:px-2 md:px-3 lg:px-4 py-2 text-center responsive-text-sm">€{item?.itemPrice?.toFixed(2) ?? "0.00"}</td>
            <td className="px-1 sm:px-2 md:px-3 lg:px-4 py-2 text-center responsive-text-sm" >€{(item?.quantity * item?.itemPrice).toFixed(2) ?? "0.00"}</td>
          </tr>
        ))} 
            </tbody>
          </table>
