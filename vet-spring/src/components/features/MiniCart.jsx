        ...
        {isOpen && (
          <div className="w-60 sm:w-80  md:w-100  bg-emerald-100/10 backdrop-blur-md border border-emerald-200/100 rounded-xl shadow-lg z-50 p-2 sm:p-3 md:p-4 flex flex-col">
            <div className="flex justify-between items-center mb-2 sm:mb-2.5 md:mb-3">
              <h2 className="responsive-text-md text-emerald-900 font-bold">
                Your Cart
              </h2>
              <p className="responsive-text-sm text-orange-800">
                <span>Total: </span>${totalSum?.toFixed(2)}
              </p>
            ...
            <div className="flex-1 max-h-50 sm:max-h-55 md:max-h-60 overflow-y-auto space-y-2 sm:space-y-2.5 md:space-y-3">
              {cartItems?.length === 0 && (
                <p className="responsive-text-sm text-info-content">
                  Cart is empty
                </p>
              )}
            ...
            <div className="flex items-center justify-between ">
              <button
                type="button"
                className="responsive-text-sm mt-2 sm:mt-3 md:mt-4 text-red-800 cursor-pointer rounded-md px-1 sm:px-1.5 md:px-2 py-0.25 md:py-0.5 hover:font-semibold hover:scale-105 duration-100"
                onClick={clearCart}
              >
                Clear Cart
              </button>
              <button
                ...
                className={`responsive-text-sm mt-2 sm:mt-3 md:mt-4 py-1 sm:py-1.5 md:py-2 w-6/10 mx-2 rounded-md
                  ${
                    !Array.isArray(cartItems) || cartItems.length === 0
                      ? "bg-gray-400 text-white cursor-not-allowed opacity-60"
                      : "bg-gradient-to-br from-sky-700 via-sky-800 to-info-content text-white hover:from-sky-500 hover:via-sky-600 hover:to-sky-700 cursor-pointer"
                  }`}
              >
